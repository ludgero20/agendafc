// lib/services/nfl-service.ts
import fs from 'fs/promises';
import path from 'path';
import { JogoNFL } from '@/app/components/RodadaNFLClient';
...
export function identificarDivisaoNFL(nomeTime: string) {
  for (const [timeNome, info] of Object.entries(divisoesOficiaisNFL)) {
    if (
      nomeTime.toLowerCase().includes(timeNome.toLowerCase()) ||
      timeNome.toLowerCase().includes(nomeTime.toLowerCase())
    ) {
      return info;
    }
  }
...
// 1. CLASSIFICAÇÃO NFL (MULTI-URL COM FAILOVER)
export async function getTabelaCompletaNFL(): Promise<TimeTabelaNFL[] | null> {
  const urls = [
    ...
    'https://site.api.espn.com/apis/v2/sports/football/nfl/standings?region=us&lang=en&season=2026&type=2',
  ];
        ...
        if (obj.standings?.entries && Array.isArray(obj.standings.entries)) {
          obj.standings.entries.forEach((entry: any) => {
            ...
            const getStat = (n: string) =>
              stats.find(
                (x: any) =>
                  x.name?.toLowerCase() === n.toLowerCase() || x.type?.toLowerCase() === n.toLowerCase()
              )?.value ?? 0;
            ...
            const logoUrl =
              ...
              entry.team?.logo ||
              (abbrev ? `https://a.espncdn.com/i/teamlogos/nfl/500/${abbrev}.png` : '');
...
export async function getTodosJogosNFL(): Promise<JogoNFL[] | null> {
  const dominios = ['https://site.web.api.espn.com', 'https://site.api.espn.com'];
  ...
  for (const dominio of dominios) {
    try {
      ...
      const responses = await Promise.all(
        semanas.map((semana) =>
          fetch(
            `${dominio}/apis/site/v2/sports/football/nfl/scoreboard?dates=2026&seasontype=2&week=${semana}`,
            {
              headers: ESPN_HEADERS,
              next: { revalidate: 1800 },
            }
      ...
      responses.forEach((data, index) => {
        ...
        eventos.forEach((ev: any) => {
          ...
          const strTime = dataObj.toLocaleTimeString('pt-BR', {
            hour: '2-digit',
            minute: '2-digit',
          ...
          const finalizado = Boolean(
            ev.status?.type?.completed || ev.status?.type?.state === 'post' || ev.status?.type?.name === 'STATUS_FINAL'
          );
          ...
          const logoHome =
            ...
            `https://a.espncdn.com/i/teamlogos/nfl/500/${home?.team?.abbreviation?.toLowerCase() || 'nfl'}.png`;
          const logoAway =
            away?.team?.logo ||
            away?.team?.logos?.[0]?.href ||
            `https://a.espncdn.com/i/teamlogos/nfl/500/${away?.team?.abbreviation?.toLowerCase() || 'nfl'}.png`;

          const homeScore = home?.score !== undefined && home?.score !== '' ? String(home.score) : '0';
          const awayScore = away?.score !== undefined && away?.score !== '' ? String(away.score) : '0';
          ...
          todosJogos.push({
            ...
            intHomeScore: finalizado || emAndamento ? homeScore : null,
            intAwayScore: finalizado || emAndamento ? awayScore : null,
...
// 3. JOGOS DA NFL ESPECÍFICOS DE UMA FRANQUIA
export async function getJogosNFLDoTime(
  time: TimeConfig
): Promise<{ finalizados: JogoTemporadaNFL[]; proximos: JogoTemporadaNFL[] }> {
  let finalizados: JogoTemporadaNFL[] = [];
  let proximos: JogoTemporadaNFL[] = [];

  try {
    const todosJogos = await getTodosJogosNFL();
    if (!todosJogos) return { finalizados, proximos };

    const jogosDoTime: JogoTemporadaNFL[] = [];

    todosJogos.forEach((j) => {
      const ehJogoDoTime = time.variacoesNome.some(
        (v) =>
          j.strHomeTeam.toLowerCase().includes(v.toLowerCase()) ||
          j.strAwayTeam.toLowerCase().includes(v.toLowerCase())
      );

      if (ehJogoDoTime) {
        const finalizado = j.strStatus === 'Match Finished';
        jogosDoTime.push({
          id: j.idEvent,
          dateStr: `${j.dateEvent} às ${j.strTime}`,
          status: finalizado ? 'FINISHED' : 'SCHEDULED',
          roundLabel: `Semana ${j.intRound}`,
          homeTeam: { name: j.strHomeTeam, shortName: j.strHomeTeam, crest: j.strHomeLogo || '',},
          awayTeam: { name: j.strAwayTeam, shortName: j.strAwayTeam, crest: j.strAwayLogo || '',},
          homeScore: j.intHomeScore,
          awayScore: j.intAwayScore,
        });
      }
    });

    finalizados = jogosDoTime.filter((j) => j.status === 'FINISHED').slice(-3);
    proximos = jogosDoTime.filter((j) => j.status !== 'FINISHED').slice(0, 5);
  } catch {}
