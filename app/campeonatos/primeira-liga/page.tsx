...
export const metadata: Metadata = {
  title: "Tabela e Jogos da Primeira Liga | Agenda FC",
  description: "Acompanhe a tabela de classificação e os próximos jogos da Primeira Liga. Veja a situação do seu time e a agenda completa da rodada do campeonato português.",
};
...
async function getTabelaPrimeiraLiga(): Promise<Tabela | null> {
  try {
    const filePath = path.join(process.cwd(), "public/api-cache/primeira-liga-standings.json");
    const jsonData = await fs.readFile(filePath, "utf-8");
    const data = JSON.parse(jsonData);
    return data?.standings?.[0]?.table;
  } catch (error) {
    console.error("ERRO AO LER ARQUIVO de cache da tabela (Primeira Liga):", error);
    return null;
  }
}
...
async function getProximosJogosPrimeiraLiga(): Promise<ProximoJogo[] | null> {
  try {
    const filePath = path.join(process.cwd(), "public/api-cache/primeira-liga-matches.json");
    const jsonData = await fs.readFile(filePath, "utf-8");
    const data = JSON.parse(jsonData);
    return data?.matches;
  } catch (error) {
    console.error("ERRO AO LER ARQUIVO de cache dos jogos (Primeira Liga):", error);
    return null;
  }
}

// --- Componente da Página ---
export default async function PrimeiraLigaPage() {
  const [tabela, proximosJogos] = await Promise.all([ getTabelaPrimeiraLiga(), getProximosJogosPrimeiraLiga() ]);

  if (!tabela) {
    return ( <div className="bg-red-50 border border-red-200 text-red-800 p-6 rounded-lg"> <h2 className="font-bold text-lg mb-2">Erro ao Carregar a Tabela</h2> <p>Não foi possível buscar os dados da classificação da Primeira Liga no momento.</p> </div> );
  }
  ...
  return (
    <div>
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold">Primeira Liga</h1>
        <p className="text-xl text-gray-600 mt-2">Campeonato Português</p>
      </div>
