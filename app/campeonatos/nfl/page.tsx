...
async function getTabelaNFL(): Promise<TimeTabelaNFL[] | null> {
  ...
  for (const url of urls) {
    try {
      ...
      console.error("Tentando próxima URL de classificação NFL...");
    }
  }
  ...
  for (const dominio of dominios) {
    try {
      ...
      const responses = await Promise.all(
        semanas.map(semana =>
          fetch(`${dominio}/apis/site/v2/sports/football/nfl/scoreboard?dates=2026&seasontype=2&week=${semana}`, {
            headers: {
              'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36',
              'Accept': 'application/json',
              'Referer': 'https://www.espn.com/'
      ...
      responses.forEach((data, index) => {
        ...
        eventos.forEach((ev: any) => {
          ...
          const finalizado = Boolean(ev.status?.type?.completed || ev.status?.type?.state === 'post' || ev.status?.type?.name === 'STATUS_FINAL');
      ...
      console.error("Tentando próximo domínio ESPN NFL...");
    }
  }
