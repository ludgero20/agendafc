...
async function getTabelaSerieA(): Promise<Tabela | null> {
try {
    const filePath = path.join(process.cwd(), "public/api-cache/serie-a-standings.json");
    const jsonData = await fs.readFile(filePath, "utf-8");
    const data = JSON.parse(jsonData);
    return data?.standings?.[0]?.table;
  } catch (error) {
    console.error("ERRO AO LER ARQUIVO de cache da tabela (Serie A):", error);
    return null;
  }
}

async function getProximosJogosSerieA(): Promise<ProximoJogo[] | null> {
try {
    const filePath = path.join(process.cwd(), "public/api-cache/serie-a-matches.json");
    const jsonData = await fs.readFile(filePath, "utf-8");
    const data = JSON.parse(jsonData);
    return data?.matches;
  } catch (error) {
    console.error("ERRO AO LER ARQUIVO de cache dos jogos (Serie A):", error);
    return null;
  }
}
...
export default async function SerieAPage() {
  const [tabela, proximosJogos] = await Promise.all([ getTabelaSerieA(), getProximosJogosSerieA() ]);

  if (!tabela) {
    return ( <div className="bg-red-50 border border-red-200 text-red-800 p-6 rounded-lg"> <h2 className="font-bold text-lg mb-2">Erro ao Carregar a Tabela</h2> <p>Não foi possível buscar os dados da classificação da Serie A no momento.</p> </div> );
  }
  ...
  return (
    <div>
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold">Serie A</h1>
        <p className="text-xl text-gray-600 mt-2">Campeonato Italiano</p>
      </div>
