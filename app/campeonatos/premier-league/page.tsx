...
export const metadata: Metadata = {
  ...
  description: "Acompanhe a tabela de classificação e os próximos jogos da Premier League. Veja a situação do seu time e a agenda completa da rodada do campeonato inglês.",
};
...
export default async function PremierLeaguePage() {
  const [tabela, proximosJogos] = await Promise.all([
    getTabelaPremierLeague(),
    getProximosJogosPremierLeague()
  ]);
  ...
  if (!tabela) {
    return (
        <div className="bg-red-50 border border-red-200 text-red-800 p-6 rounded-lg">
            ...
            <p>Não foi possível buscar os dados da classificação da Premier League no momento.</p>
        </div>
    );
  }
  ...
  return (
    <div>
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold">Premier League</h1>
        <p className="text-xl text-gray-600 mt-2">Campeonato Inglês</p>
      </div>
