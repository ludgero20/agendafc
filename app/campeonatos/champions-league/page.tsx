...
async function getTabelaChampionsLeague(): Promise<Tabela | null> {
  ...
  if (!res.ok) {
    console.error("ERRO AO BUSCAR TABELA (Champions League)");
    return null;
  }

  const data = await res.json();
  // Com o novo formato, a tabela principal é a primeira (e única) do array
  return data?.standings?.[0]?.table;
}

async function getProximosJogosChampions(): Promise<ProximoJogo[] | null> {
  // Usamos o filtro 'status=SCHEDULED' para pegar apenas jogos futuros
  ...
  if (!res.ok) {
    console.error("ERRO AO BUSCAR JOGOS (Champions League)");
    return null;
  }
